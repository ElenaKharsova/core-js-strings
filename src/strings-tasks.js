...
function findLongestWord(sentence) {
  const arrLengths = sentence
    .split(' ')
    .map((el) => ({ length: el.length, word: el }))
    .sort((a, b) => b.length - a.length);
  return arrLengths[0].word;
}
...
function invertCase(str) {
  function revertChar(el) {
    return el === el.toUpperCase() ? el.toLowerCase() : el.toUpperCase();
  }
  return str.split('').map(revertChar).join('');
}
...
function extractNameFromTemplate(value) {
  const [, name] = value.slice(0, -1).split(', ');
  return name;
}
...
function encodeToRot13(str) {
  const cipher = 'AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz';
  function createChipher(el) {
    const index = cipher.indexOf(el);
    if (index === -1) {
      return el;
    }
    return cipher.at((26 + index) % cipher.length);
  }
  return str.split('').map(createChipher).join('');
}
